...
| 인스턴스 변수(instance variables) |                     클래스 영역                     | **인스턴스(객체)가 생성되었을 때**( 이게 가장 중요함!, 즉 객체가 생성이 안되어도 가능함! ) |
...
- 기본 생성자
  ...
  이렇게 있으면 에러가 난다. ==> ??? 뭔 소리냐 에러 안남 갓궁성이 얘기했던건 왠만하면 기본 생성자는 클래스 만들 때 항상 만들라고 했고, 안 이러면 자주 에러가 난다고 했었다. 컴파일러 믿지 말고 애진작 습관들이자가 key였다. 위처럼 썼을 때 에러가 나는 경우는 다음과 같을 때이다.
